...
| 组件 | 启动命令 | 端口 | 依赖 |
|------|---------|------|------|
| 前端 | `cd frontend && npm run dev` | 3000 | 后端 localhost:8080 |
| 后端 | `cd backend && ./gradlew bootRun` | 8080 | H2 内存数据库（自动） |
| H2 Console | 浏览器访问 `/h2-console` | 8080 | JDBC URL: `jdbc:h2:mem:visitalk` |

- 后端使用 H2 内存数据库：每次重启自动建表（`create-drop`）+ `DataInitializer` 种测试用户，无需安装 Supabase。
- 测试账号：`parent@test.com` / `child@test.com`，密码均为 `password123`。
- 前端登录后 JWT token 存 `localStorage`（key: `visitalk_token`），开发时可在 DevTools → Application → Local Storage 中查看或清除。
...
- CI 配置文件：`.github/workflows/ci.yml`；前端 lint/单元测试使用 Vitest，后端使用 JUnit 5 + H2 测试数据库。
...
| 单元测试 (后端) | JUnit 5 + H2        | Service 业务逻辑、周报聚合算法           | 后端开发者      | 本地 + CI      |
| 集成 / RLS | JUnit 5 + H2 / Supabase | API 端到端、**RLS 越权防护**          | 后端开发者      | CI           |
...
### 9.2 RLS / 安全测试 (强制)
- 必须验证：`child` 会话访问 `BehaviorEvent` 被拒绝。
- 必须验证：`parent` 会话读取 `DiaryEntry` 明细字段被拒绝、只能读聚合。
